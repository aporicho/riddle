...
expected_content_id=$(
    {
        ...
        printf '%s\0%s\0%s\0' magicpaper magicpaper 0.7.1
        printf '%s\0%s\0%s\0%s\n' \
...
python3 "$ROOT/scripts/remagic-bundle.py" create "$CANONICAL" \
    --app-id magicpaper --package magicpaper --version 0.7.1
...
ARCHIVE=$OUT/magicpaper-0.7.1-universal_aarch64.tar.gz
[ -s "$ARCHIVE" ]
...
if python3 "$ROOT/scripts/remagic-bundle.py" verify "$TMP/extracted" \
    --app-id magicpaper --package magicpaper --version 0.7.1 >/dev/null 2>&1; then
